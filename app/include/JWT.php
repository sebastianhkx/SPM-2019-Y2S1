...
class JWT
{
	...
	// addition functions
	/**
	 * Returns a JSON web token based on the $username and $secret_key.
	 * 
	 * @param string $username The username of the logged in user
	 * @param string $secret_key The shared secret
	 * 
	 ...
	 * @param string $secret_key The shared secret
	 * 
	 * @return mixed If this is a valid token, return the username (string).  
	...
	public static function verify_token($token, $secret_key) {
		try {
			...
			$date = DateTime::createFromFormat( 'Y-m-d H:i:s', $json_object->{'datetime'});
			$date = $date->getTimestamp();
			$diff = $date - $now;

			//echo "$diff seconds elapsed\n";
			if ( $diff < 60 * 60 * 24) {  // expire after 1 day 
